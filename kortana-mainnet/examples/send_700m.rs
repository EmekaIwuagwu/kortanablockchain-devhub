...
fn main() {
    println!("🚀 KORTANA MAINNET TREASURY DISBURSEMENT - 700M DNR");
    println!("RPC: {}", RPC_URL);
    println!("--------------------------------------");
    ...
    let amount_dnr = 700_000_000u128;
    ...
    let nonce = if let Some(n_str) = nonce_resp.as_str() {
        ...
        println!("⚠️ Warning: Could not fetch nonce from RPC. Defaulting to 0.");
        0
    };
    ...
    if tx_hash.is_null() || tx_hash.as_str().is_none() {
        ...
        println!("Check if the local node is running at http://127.0.0.1:8545");
        return;
    }
    ...
    if receipt.is_null() {
        ...
        if status_hex == "0x1" {
            println!("🎉 SUCCESS: 700M DNR sent and confirmed!");
        } else {
